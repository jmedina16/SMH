...
Content.prototype = {
    ...
    getUiConfs: function () {
        ...
        var reqUrl = "http://meddev2/index.php/kmc/getuiconfs";
        $.ajax({
            cache: false,
            url: reqUrl,
            async: false,
    ...
    previewEmbed: function (entryId, name, platforms) {
        ...
        player_prev_gen = smhContent.generateEmbed(uiconf_id, entryId, width, height, 'http', delivery, embed, seo, name, true, sizing, ratio);
    ...
    getShortLink: function (uiconf, entryid, embed, delivery) {
        var cb = function (success, results) {
            ...
            $('#qrcode').empty();
            shortlink = 'http://meddev2/tiny/' + results['id'];
        ...
        var url = '';
        if (delivery == 'http') {
            url = "http://meddev2/index.php/extwidget/preview/partner_id/" + sessInfo.pid + "/uiconf_id/" + uiconf + "/entry_id/" + entryid + "/embed/" + embed + "?&flashvars[ks]=" + sessInfo.ks;
        } else {
            url = "http://meddev2/index.php/extwidget/preview/partner_id/" + sessInfo.pid + "/uiconf_id/" + uiconf + "/entry_id/" + entryid + "/embed/" + embed + "?&flashvars[streamerType]=rtmp&flashvars[mediaProtocol]=rtmp&flashvars[ks]=" + sessInfo.ks;
        }
    ...
    generateEmbed: function (uiconf_id, entryId, width, height, protocol, streamerType, embed, seo, name, preview, sizing, aspectRatio) {
        ...
        var gen = new kEmbedCodeGenerator({
            ...
            entryMeta: {
                name: name,
                thumbnailUrl: "http://meddev2/p/" + sessInfo.pid + "/sp/" + sessInfo.pid + "00/thumbnail/entry_id/" + entryId + "/width/120/height/90/bgcolor/000000/type/2",
                width: width,
                height: height
            }
        });
...
//            }
    ...
    editThumbnail: function (id, type) {
        ...
        content = '<iframe style="width:0px;height:0px;border:0px;position: absolute;" name=hiddenIFrame />' +
                '<form id="thumbUp" enctype="multipart/form-data" target="hiddenIFrame" method="post" action="http://meddev2/api_v3/index.php?service=thumbAsset&action=addfromimage">' +
        ...
        var cb = function (success, results) {
            ...
            $.each(myData, function (key, value) {
                ...
                if (value['tags'] == "default_thumb") {
                    image = "<div style='height: 100px;'><span class='helper'></span><img src='http://meddev2/api_v3/service/thumbAsset/action/serve/ks/" + sessInfo.ks + "/thumbAssetId/" + value['id'] + "' style='width: 130px; max-height: 100px; vertical-align: middle;' /></div>";
                    ...
                    actions = '<span class="dropdown header">' +
                            ...
                            '<li role="presentation"><a href="http://meddev2/api_v3/service/thumbAsset/action/serve/thumbAssetId/' + value['id'] + '/options:download/true" tabindex="-1" role="menuitem">Download</a></li>' +
                            '</ul>' +
                            '</div>' +
                            '</span>';
                } else {
    ...
    exportMetaData: function () {
        if (total_entries) {
            window.location = 'http://meddev2/apps/platform/metadata/export.metadata.php?pid=' + sessInfo.pid + '&ks=' + sessInfo.ks + '&page_size=' + total_entries + '&action=export_entries_metadata';
        }
    },
    ...
    clipEntry: function (id) {
        ...
        content = '<iframe id="trim-clip" scrolling="no" width="100%" height="100%" frameborder="0" src="http://meddev2/apps/platform/clipApp/v1.0.1s/index.php?kdpUiconf=23448205&kclipUiconf=23448173&partnerId=' + sessInfo.pid + '&mode=clip&config=kmc&entryId=' + id + '&uid=' + sessInfo.Id + '&ks=' + sessInfo.ks + '">';
        $('#smh-modal .modal-body').html(content);
    ...
    trimEntry: function (id) {
        ...
        content = '<iframe id="trim-clip" scrolling="no" width="100%" height="100%" frameborder="0" src="http://meddev2/apps/platform/clipApp/v1.0.1s/index.php?kdpUiconf=23448205&kclipUiconf=23448173&partnerId=' + sessInfo.pid + '&mode=trim&config=kmc&entryId=' + id + '&uid=' + sessInfo.Id + '&ks=' + sessInfo.ks + '">';
        $('#smh-modal .modal-body').html(content);
    ...
    loadFlavors: function (data) {
        ...
        var i = 0;
        $.each(data, function (key, value) {
            if (value['flavorAsset'] === undefined && value['flavorParams']['objectType'] === 'KalturaFlavorParams') {
                ...
                if (value['flavorAsset']) {
                    ...
                    videoCodec = (value['flavorAsset']['videoCodecId']) ? value['flavorAsset']['videoCodecId'] : '';
    ...
    uploadFlavorModal: function (entryid, paramId, name) {
        ...
        content = '<iframe id="upload_form" scrolling="no" width="100%" height="100%" frameborder="0" src="http://meddev2/apps/platform/FlavorUpload.php?eid=' + entryid + '&fid=' + paramId + '&ks=' + sessInfo.ks + '">';
        $('#smh-modal2 .modal-body').html(content);
        ...
        if (lang == 'none') {
            ...
            var cb = function (success, results) {
                ...
                content = '<div style="width: 425px; margin-left: auto; margin-right: auto;"><iframe style="width:0px;height:0px;border:0px;" name=hiddenIFrame />' +
                        '<form id="capUpload" enctype="multipart/form-data" target="hiddenIFrame" method="post" action="http://meddev2/api_v3/index.php?service=uploadtoken&action=upload">' +
                        '<input type="hidden" name="ks" value="' + sessInfo.ks + '">' +
