...
Playlists.prototype = {
    ...
    getUiConfs: function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                uiconf_ids = data;
                var index;
                index = smhMain.getIndex(uiconf_ids, '23448207');
                uiconf_ids.splice(index, 1);
                index = smhMain.getIndex(uiconf_ids, '23448208');
                uiconf_ids.splice(index, 1);
                ...
                array2['width'] = "400";
                array2['height'] = "680";
                uiconf_ids.unshift(array1, array2);
            }
        });
    },
    ...
    previewPlaylist: function (entryId, name) {
        ...
        player_prev_gen = smhPlaylists.generatePlaylistEmbed(uiconf_id, entryId, width, height, 'http', delivery, embed, seo, name, true, sizing, ratio);
...
//                    }
    ...
    loadEntries_manual_plist: function () {
        ...
        manualTable = $('#smh-modal #plist-data').DataTable({
            ...
            "scrollY": "511px",
            "ajax": {
                "url": "/api/v1/getPlaylists_EntriesFW",
                "type": "GET",
                "data": function (d) {
                    return $.extend({}, d, {
                        "_token": $('meta[name="csrf-token"]').attr('content'),
    ...
    loadEntries_rb_plist: function () {
        ...
        rbTable = $('#smh-modal #plist-data').DataTable({
            ...
            "scrollY": "511px",
            "ajax": {
                "url": "/api/v1/getRBPlaylists_EntriesFW",
                "type": "GET",
                "data": function (d) {
                    return $.extend({}, d, {
                        "_token": $('meta[name="csrf-token"]').attr('content'),
    ...
    generatePlaylistEmbed: function (uiconf_id, entryId, width, height, protocol, streamerType, embed, seo, name, preview, sizing, aspectRatio) {
        ...
        var gen = new kEmbedCodeGenerator({
            ...
            entryMeta: {
                name: name,
                thumbnailUrl: protocol + "://meddev2/p/" + sessInfo.pid + "/sp/" + sessInfo.pid + "00/thumbnail/entry_id/" + entryId + "/width/120/height/90/bgcolor/000000/type/2",
                width: width,
                height: height
            }
        });
...
//                    }
        ...
        var cb = function (success, results) {
            ...
            $.each(results, function (index, value) {
                ...
                var entry_container = "<div class='entry-wrapper ui-draggable hover' data-entryid='" + value.id + "' data-duration='" + duration_data + "'>" +
                        ...
                        "<img src='http://meddev2/p/" + value.partnerId + "/thumbnail/entry_id/" + value.id + "/quality/100/type/1/width/300/height/90' width='100' height='68'>" +
                        "</div>" +
    ...
    exportMetaData: function () {
        if (total_entries) {
            window.location = 'http://meddev2/apps/platform/metadata/export.metadata.php?pid=' + sessInfo.pid + '&ks=' + sessInfo.ks + '&page_size=' + total_entries + '&action=export_plist_metadata';
        }
    },
