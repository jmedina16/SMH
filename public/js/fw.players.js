...
Players.prototype = {
    ...
    renderPlayer: function () {
        var player_prev_gen = smhPlayers.generateEmbed(player_entryid, flashvars['width'], flashvars['height'], 'http', 'dynamic', false, 'Player Preview', true);
    ...
    getShortLink: function (uiconf, embed, delivery) {
        ...
        var url = '';
        if (delivery == 'http') {
            url = "http://meddev2/index.php/extwidget/preview/partner_id/" + sessInfo.pid + "/uiconf_id/" + uiconf + "/embed/" + embed + "?&flashvars[ks]=" + sessInfo.ks;
        } else {
            url = "http://meddev2/index.php/extwidget/preview/partner_id/" + sessInfo.pid + "/uiconf_id/" + uiconf + "/embed/" + embed + "?&flashvars[streamerType]=rtmp&flashvars[mediaProtocol]=rtmp&flashvars[ks]=" + sessInfo.ks;
        }
    ...
    generateEmbed: function (entryId, width, height, protocol, embed, seo, name) {
        var cacheSt = smhPlayers.getCacheSt();
        if (uiconf_id == '23448210' || uiconf_id == '23448211') {
            var gen = new kEmbedCodeGenerator({
                host: "meddev2",
                securedHost: "meddev2",
                embedType: embed,
                ...
                entryMeta: {
                    name: name,
                    thumbnailUrl: "http://meddev2/p/10364/sp/1036400/thumbnail/entry_id/" + entryId + "/width/120/height/90/bgcolor/000000/type/2",
                    width: width,
                    height: height
                }
            });
    ...
    editPlayer: function (id) {
        var cb = function (success, results) {
            ...
            if (typeof $(xml).find("layout").attr('isPlaylist') !== 'undefined') {
                uiconf_id = $(xml).find("layout").attr('isPlaylist') == "true" ? 23448210 : 23448211;
                if (uiconf_id == 23448210) {
                    flashvars['playlistAPI.kpl0Id'] = '0_632qrz3t';
                    flashvars['playlistAPI.kpl0Name'] = 'playlist';
                    flashvars['playlistAPI.kpl1Id'] = null;
                    flashvars['playlistAPI.kpl1Name'] = null;
                }
                if (uiconf_id == 23448211) {
                    $.each(plugins.playlistAPI, function (key, value) {
                        if (key.match(/kpl/g)) {
                            if (key.match(/Id/g)) {
                                TEMP_MULTI.push(value);
    ...
    createPlayer: function () {
        ...
        if (player_name == null || player_name == '') {
            ...
            var defaults = {
                animatePadding: 60,
                ApiUrl: "http://meddev2/api_v3/index.php?",
                sessSrv: "uiconf",
                sessAct: "add",
                format: "1"
            };
