...
Playlists.prototype = {
    ...
    previewPlaylist: function (entryId, name) {
        ...
        if ($.inArray("PLAYLIST_EMBED_CODE", sessPerm) != -1) {
            ...
            embed_type = '<hr>' +
                    '<div style="margin-top: 10px; font-weight: bold;"><span style="color: #444; font-size: 12px; margin-right: 37px;">Embed Type:</span><span><select class="form-control embedType" style="width: 213px;"><option value="dynamic">Dynamic Embed</option></select></span></div>' +
        ...
        if ($.inArray("PLAYLIST_EMBED_CODE", sessPerm) != -1) {
            gen = smhPlaylists.generatePlaylistEmbed(uiconf_id, entryId, width, height, protocol, delivery, format, embed, seo, err_comp, name, false, sizing, ratio);
            embedCode = gen.getCode();
            $('#embed_code').text(embedCode);
        }

        player_prev_gen = smhPlaylists.generatePlaylistEmbed(uiconf_id, entryId, width, height, 'https', delivery, format, embed, seo, err_comp, name, true, sizing, ratio);
    ...
    generatePlaylistEmbed: function (uiconf_id, entryId, width, height, protocol, streamerType, format, embed, seo, err_comp, name, preview, sizing, aspectRatio) {
        var flashvars = {};
        if (streamerType == 'hls') {
