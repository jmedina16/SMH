...
Reseller.prototype = {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data['success']) {
                        ...
                        setTimeout(function () {
                            $('#smh-modal #pass-result').empty();
                            smhRS.editServices(0, 0, 0, 0, 0, 0, 0, 0, data['child_id'], true);
                        }, 3000);
                    } else if (data['error']) {
                        $('#smh-modal #loading').empty();
    ...
    editServices: function (mb, trans, ppv, mem, vc, wl, efwl, cl, pid, ca) {
        smhMain.resetModal();
        var header, content, footer;
        var mb_service = '';
        var trans_service = '';
        ...
        var layout_service = '';
        if (services.mb) {
            var mb_check = (mb) ? 'checked' : '';
            mb_service = '<tr class="services-body"><td class="services-text">Mobile Streaming</td><td class="services-switch"><input ' + mb_check + ' data-toggle="toggle" id="mb-service" type="checkbox"></td></tr>';
        }
    ...
    modifyUserLayout: function (pid) {
        ...
        var cl_service = 0;
        if (services.mb) {
            mb_service = ($('#service-table #mb-service').prop('checked')) ? 1 : 0;
        }
        if (services.trans_vod) {
        ...
        footer = '<div id="layout-result"></div><div id="loading"><img height="20px" src="/img/loading.gif"></div><button type="button" class="btn btn-default" onclick="smhRS.editServices(' + mb_service + ',' + trans_service + ',' + ppv_service + ',' + mem_service + ',' + vc_service + ',' + wl_service + ',' + efwl_service + ',' + cl_service + ',' + pid + ',' + false + ');">Back</button><button type="button" class="btn btn-primary" id="save-account-layout" onclick="smhRS.saveAcntLayout(' + mb_service + ',' + trans_service + ',' + ppv_service + ',' + mem_service + ',' + vc_service + ',' + wl_service + ',' + efwl_service + ',' + cl_service + ',' + pid + ')">Save</button>';
        $('#smh-modal .modal-footer').html(footer);
    ...
    saveAcntLayout: function (mb_service, trans_service, ppv_service, mem_service, vc_service, wl_service, efwl_service, cl_service, pid) {
            ...
            success: function (data) {
                ...
                if (data['success']) {
                    ...
                    setTimeout(function () {
                        ...
                        smhRS.editServices(mb_service, trans_service, ppv_service, mem_service, vc_service, wl_service, efwl_service, cl_service, pid, false);
                    }, 2000);
                }
            }
        });
