...
Content.prototype = {
    ...
    previewEmbed: function (entryId, name, platforms) {
        ...
        if ($.inArray("CONTENT_MANAGE_EMBED_CODE", sessPerm) != -1) {
            ...
            embed_type = '<div id="embed-types-options">' +
                    '<div style="margin-top: 10px; font-weight: bold;"><span style="color: #444; font-size: 12px; margin-right: 37px;">Embed Type:</span><span><select class="form-control embedType" style="width: 213px;"><option value="dynamic">Dynamic Embed</option><option value="thumb">Thumbnail Embed</option></select></span></div>' +
            ...
            secure_seo = '<div id="seo-options">' +
                    ...
                    '<hr>' +
                    '</div>';
            vr_fallback = '<div id="vr-options">' +
                    '<div style="margin-top: 10px; font-weight: bold;"><span style="color: #444; font-size: 12px; padding-top: 15px; margin-right: 15px;">Fallback EntryId:</span><span style="font-weight: normal; font-size: 15px;"><a href="#" id="update-fb-eid" onclick="smhContent.selectFBModal();">None Selected</a></span></div>' +
                    ...
                    '<hr>' +
                    '</div>';
            vr_comp_mess = '<div id="comp-options">' +
                    '<div style="margin-top: 10px; font-weight: bold;"><span><input type="checkbox" id="error-comp" name="error_comp" checked></span>&nbsp;<span style="color: #444; font-size: 12px; font-weight: bold; margin-left: 5px; position: relative; top: -2px;">Display Browser Compatibility Warning</span></div>' +
        ...
        content = '<div class="content">' +
                ...
                '<div style="margin-top: 10px; font-weight: bold;"><span style="color: #444; font-size: 12px;margin-right: 11px;">VR Video Format:</span><span><select class="form-control format" style="width: 213px;"><option value="2d">2D</option><option value="sbs">Side By Side</option><option value="tb">Top/Bottom</option></select></span></div>' +
        ...
        if ($.inArray("CONTENT_MANAGE_EMBED_CODE", sessPerm) != -1) {
            gen = smhContent.generateEmbed(uiconf_id, entryId, width, height, protocol, delivery, format, embed, seo, err_comp, name, false, sizing, ratio);
            embedCode = gen.getCode();
            $('#embed_code').text(embedCode);
        }

        player_prev_gen = smhContent.generateEmbed(uiconf_id, entryId, width, height, 'https', delivery, format, embed, seo, err_comp, name, true, sizing, ratio);
        ...
        $('#smh-modal3').on('change', 'select#platforms', function (event) {
            ...
            if (selected_platform == 'smh') {
                ...
                $('#vr-format-options').css('display', 'block');
                $('#vr-options').css('display', 'block');
                $('#comp-options').css('display', 'block');
                ...
                $('#vr-format-options').css('display', 'none');
                $('#vr-options').css('display', 'none');
                $('#comp-options').css('display', 'none');
        ...
        if ($.inArray("CONTENT_MANAGE_EMBED_CODE", sessPerm) != -1) {
            ...
            $('.previewModal .options').on('change', '#error-comp', function (event) {
    ...
    //Select VR fallback Entry ID
    selectFBModal: function () {
        smhContent.resetModal();
        smhContent.resetFilters();
        ...
        var tree_flavors = smhContent.json_tree(flavors, 'flavors');
    ...
    editPlatformConfig: function (eid, platforms, stereo_mode) {
        ...
        if (!yt_ready && !fb_ready && !twch_ready) {
            ...
            content = '<div style="color: #797979; text-align: center;">Enable the platforms you would like to upload this video to.</div>' +
                    ...
                    '<div class="sn-choice-wrapper"><div style="margin-right: 35px;float: left;position: relative;top: inherit;font-weight: bold;">VR Video Format:</div>' +
                    '<div style="float: left;position: relative;top: -5px;width: 235px;">' +
                    '<select id="vr-select" class="form-control">' +
                    '<option value="none">2D</option>' +
                    '<option value="top-bottom">3D Top/Bottom</option>' +
                    '<option value="left-right">3D Side By Side</option>' +
                    '</select>' +
                    '</div>' +
                    '</div>' +
    ...
    doSnUpdate: function (eid) {
        ...
        var vr_stereo_mode = $('#vr-select').val();
        if (yt_ready || fb_ready || twch_ready) {
            if (yt_ready) {
    ...
    //Generate embed code
    generateEmbed: function (uiconf_id, entryId, width, height, protocol, streamerType, format, embed, seo, err_comp, name, preview, sizing, aspectRatio) {
        var flashvars = {};
        ...
        if (format == '2d') {
            ...
            if (!err_comp) {
                flashvars.smhVR["showCompMessage"] = err_comp;
            }
            if (fb_eid) {
    ...
    editThumbnail: function (id, type) {
        ...
        var grab_thumb = '';
        if (type == 'Video') {
            grab_thumb = '<li role="presentation"><a onclick="smhContent.grabFromVideo(\'' + id + '\')" href="#" tabindex="-1" role="menuitem"><span class="user-icon icon-film2"> Grab from Video</span></a></li>';
        }
    ...
    grabFromVideo: function (vid) {
        ...
        content = '<div style="width: 445px; margin-left: auto; margin-right: auto;"><div style="width: 350px; margin-left: auto; margin-right: auto;">' + thumbPlayer + '</div><div style="font-size: 12px; padding: 10px;">Pause the video on the appropriate frame and click the thumbnail capture button</div></div>';
        $('#smh-modal2 .modal-body').html(content);
    ...
    uploadFlavorModal: function (entryid, paramId, name) {
        ...
        $('#smh-modal2 .modal-body').css('height', '160px');
        $('#smh-modal2').modal({
            backdrop: 'static'
        });
        $('#smh-modal2').css('z-index', '2000');
        ...
        content = '<iframe id="upload_form" scrolling="no" width="100%" height="100%" frameborder="0" src="/apps/platform/FlavorUpload.php?eid=' + entryid + '&fid=' + paramId + '&ks=' + sessInfo.ks + '">';
        $('#smh-modal2 .modal-body').html(content);
