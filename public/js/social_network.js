...
var weibo_auth_redirect_url = null;
var pop_up_timer;
var status_timer;
...
//                        if (value['authorized']) {
//                            $('#weibo-table .sn-settings').css('width', '160px');
//                            $('#weibo-table #sn-resync').css('display', 'block');
//                            weibo_authorized = true;
//                            $('#weibo-status').html('<div class="green led" data-placement="top" data-toggle="tooltip" data-delay=\'{"show":700, "hide":30}\' data-original-title="Connected"></div>');
//                            $('#weibo-account').html('<div class="alert sn-account" role="alert"><div id="account-img"><img src="' + value['user_details']['user_thumb'] + '" class="img-circle" alt="Account Image" width="70px"></div><div id="account-title">' + value['user_details']['user_name'] + '</div><div id="account-button"><button id="weibo-auth" class="btn btn-weibo" type="button" onclick="smhSN.confirm_remove_weibo_auth();">Disconnect</button></div><div class="clear"></div></div>');
//                        } else {
//                            $('#weibo-table .sn-settings').css('width', '95px');
//                            $('#weibo-table #sn-resync').css('display', 'none');
//                            weibo_authorized = false;
//                            weibo_auth_redirect_url = value['redirect_url'];
//                            $('#weibo-status').html('<div class="red led" class="green led" data-placement="top" data-toggle="tooltip" data-delay=\'{"show":700, "hide":30}\' data-original-title="Not Connected"></div>');
//                            $('#weibo-account').html('<button id="weibo-auth" class="btn btn-primary" type="button" onclick="smhSN.weibo_auth_button();">Connect Account</button>');
//                        }
//                    }
                    $('#weibo-status').html('<div class="red led" class="green led" data-placement="top" data-toggle="tooltip" data-delay=\'{"show":700, "hide":30}\' data-original-title="Not Connected"></div>');
                    $('#sf-status').html('<div class="red led" class="green led" data-placement="top" data-toggle="tooltip" data-delay=\'{"show":700, "hide":30}\' data-original-title="Not Connected"></div>');
                    $('#vimeo-status').html('<div class="red led" class="green led" data-placement="top" data-toggle="tooltip" data-delay=\'{"show":700, "hide":30}\' data-original-title="Not Connected"></div>');
                });
            }
        });
    },
    ...
    toggle_weibo_settings: function () {
        $('.weibo-setting-row').toggle();
        if (!weibo_authorized) {
            $('#weibo-account-settings').css('display', 'none');
        }
    },
    ...
    weibo_auth_button: function () {
        var width = 965;
        var height = 600;
        var left = (screen.width / 2) - (width / 2);
        var top = (screen.height / 2) - (height / 2);
        myWindow = window.open("about:blank", "weibo_auth", "location=no,scrollbars=yes,resizable=yes,toolbar=no,menubar=no,width=" + width + ",height=" + height + ",top=" + top + ",left=" + left);
        myWindow.location = weibo_auth_redirect_url;
        smhSN.pollWeiboStatus();
        smhSN.checkWeiboWindowClosed();
    },
    pollWeiboStatus: function () {
        status_timer = setInterval(function () {
            smhSN.update_auth_buttons('weibo');
        }, 10000);
    },
    checkWeiboWindowClosed: function () {
    ...
    confirm_remove_weibo_auth: function () {
        smhMain.resetModal();
        var header, content, footer;
        $('.smh-dialog').css('width', '415px');
        $('#smh-modal').modal({
        ...
        content = '<div style="font-size:13px !important; text-align: center;">Are you sure you want to remove this Weibo account?</div>';
        $('#smh-modal .modal-body').html(content);

        footer = '<div id="pass-result"></div><div id="loading"><img height="20px" src="/img/loading.gif"></div><button type="button" class="btn btn-default smh-close2" data-dismiss="modal">Close</button><button type="button" class="btn btn-primary" id="remove-weibo" onclick="smhSN.removeWeiboAuth()">Disconnect</button>';
        $('#smh-modal .modal-footer').html(footer);
    },
    removeWeiboAuth: function () {
        var sessData = {
            action: 'remove_weibo_authorization',
            pid: sessInfo.pid,
            ks: sessInfo.ks
        }
        $.ajax({
            ...
            success: function (data) {
                if (data['success']) {
                    ...
                    setTimeout(function () {
                        $('#smh-modal #pass-result').empty();
                        $('#remove-weibo').removeAttr('disabled');
                        $('#smh-modal').modal('hide');
                    }, 3000);
                } else {
                    $('#smh-modal #loading img').css('display', 'none');
...
//                    if ((value['platform'] == 'weibo') && (sn_platform == 'weibo')) {
//                        if (value['authorized']) {
//                            $('#weibo-status').html('<div class="green led" class="green led" data-placement="top" data-toggle="tooltip" data-delay=\'{"show":700, "hide":30}\' data-original-title="Connected"></div>');
//                            $('#weibo-account').html('<div class="alert sn-account" role="alert"><div id="account-img"><img src="' + value['user_details']['user_thumb'] + '" class="img-circle" alt="Account Image" width="70px"></div><div id="account-title">' + value['user_details']['user_name'] + '</div><div id="account-button"><button id="weibo-auth" class="btn btn-twtr" type="button" onclick="smhSN.confirm_remove_weibo_auth();">Disconnect</button></div><div class="clear"></div></div>');
//                            weibo_authorized = true;
//                            weibo_auth_redirect_url = null;
//                            clearInterval(pop_up_timer);
//                            clearInterval(status_timer);
//                            $('#weibo-account-settings').css('display', 'table-row');
//                        } else {
//                            $('#weibo-status').html('<div class="red led" class="green led" data-placement="top" data-toggle="tooltip" data-delay=\'{"show":700, "hide":30}\' data-original-title="Not Connected"></div>');
//                            $('#weibo-account').html('<button id="weibo-auth" class="btn btn-primary" type="button" onclick="smhSN.weibo_auth_button();">Connect Account</button>');
//                            weibo_authorized = false;
//                            weibo_auth_redirect_url = value['redirect_url'];
//                            $('#weibo-account-settings').css('display', 'none');
//                            $('#weibo-integration-warning').css('display', 'none');
//                        }
//                    }
                    $('#weibo-status').html('<div class="red led" class="green led" data-placement="top" data-toggle="tooltip" data-delay=\'{"show":700, "hide":30}\' data-original-title="Not Connected"></div>');
                    $('#sf-status').html('<div class="red led" class="green led" data-placement="top" data-toggle="tooltip" data-delay=\'{"show":700, "hide":30}\' data-original-title="Not Connected"></div>');
