...
Upload.prototype = {
    ...
    init_fileupload: function () {
        ...
        $('#fileupload').fileupload({
            ...
            beforeSend: function (e, files, index, xhr, handler, callback) {
                ...
                var sessData, file, filename, orig_filename, description, tags, refid, category, ac_profile, trans_profile, vr_stereo_mode, filesize, sessionID, sessionName, session;
                file = data.files[0];
                filename = file.name;
                description = '';
                tags = '';
    ...
    saveEntry: function () {
        var entry_details = {
            ...
            "vr_stereo_mode": $('#customize-media #vr-select').val()
        }
    ...
    customizeEntry: function (e, name) {
        ...
        if (data.entry_details == undefined) {
            ...
            vr_val = data.entry_details.vr_stereo_mode;
        }
        ...
        vr_select = '<option value="none">2D</option>' +
        ...
        content = '<div id="pass-wrapper">' +
                ...
                '</tr>' +
                '<tr>' +
                '<td><b>VR Video Format:</b></td>' +
                '<td><select id="vr-select" class="form-control">' + vr_select + '</select></td>' +
                '</tr>' +
                '</table>' +
                '</form>' +
                '</div>';
