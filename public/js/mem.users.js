...
MEM.prototype = {
    ...
    getUsers: function () {
        ...
        $('#users-data').dataTable({
            ...
            "columns": [
                ...
                {
                    "title": "<span style='float: left;'>Last Login</span>"
                },
                                {
                    "title": "<span style='float: left;'>Last Logout</span>"
                },
                {
    ...
    editUserDetails: function (uid, json_details) {
        ...
        if (owner_attrs.length) {
            ...
            footer = '<div id="pass-result"></div><div id="loading"><img height="20px" src="/img/loading.gif"></div><button type="button" class="btn btn-default user-details-close" data-dismiss="modal">Close</button><button type="button" class="btn btn-primary" id="edit-user-reg" onclick="smhMEM.updateUserDetails(' + uid + ')">Update</button>';
            $('#smh-modal .modal-footer').html(footer);
    ...
    editReg: function () {
        ...
        footer = '<div id="pass-result"></div><div id="loading"><img height="20px" src="/img/loading.gif"></div><button type="button" class="btn btn-default user-reg-close" data-dismiss="modal">Close</button><button type="button" class="btn btn-primary" id="edit-user-reg" onclick="smhMEM.updateUserReg()">Update</button>';
        $('#smh-modal .modal-footer').html(footer);
    ...
    addUser: function () {
        ...
        footer = '<div id="pass-result"></div><div id="loading"><img height="20px" src="/img/loading.gif"></div><button type="button" class="btn btn-default add-user-close" data-dismiss="modal">Close</button><button id="create" class="btn btn-primary" onclick="smhMEM.createUser();">Create</button>';
        $('#smh-modal .modal-footer').html(footer);
    ...
    editUser: function (pid, uid, fname, lname, email) {
        ...
        content = '<center><form id="edit-mem-user" action="">' +
                '<table width="460px" border="0" id="admin_edit">' +
                '<tr>' +
        ...
        footer = '<div id="pass-result"></div><div id="loading"><img height="20px" src="/img/loading.gif"></div><button type="button" class="btn btn-default edit-user-close" data-dismiss="modal">Close</button><button id="update-user" class="btn btn-primary" onclick="smhMEM.updateUser(' + pid + ',' + uid + ')">Update</button>';
        $('#smh-modal .modal-footer').html(footer);

        $('#edit-mem-user input[type="text"]').tooltipster({
            trigger: 'custom',
            onlyOne: false,
            position: 'right'
        });

        validator = $("#edit-mem-user").validate({
            highlight: function (element, errorClass) {
                $(element).removeClass("valid").removeClass("error").addClass("validate-error");
            },
            unhighlight: function (element, errorClass) {
    ...
    userPassword: function (pid, email, name, status) {
        ...
        if (status == 3) {
            ...
            content = '<center>' +
                    ...
                    '</table>' +
                    '</form></center>';
            footer = '<div id="pass-result"></div><div id="loading"><img height="20px" src="/img/loading.gif"></div><button type="button" class="btn btn-default pass-close" data-dismiss="modal">Close</button><button id="reset-user-pswd" class="btn btn-primary" onclick="smhMEM.resetPswrd(' + pid + ',\'' + email + '\')">Reset</button>';
        }
        $('#smh-modal .modal-body').html(content);
        $('#smh-modal .modal-footer').html(footer);
    ...
    statusUser: function (pid, email, name, status, uid) {
        ...
        footer = '<div id="pass-result"></div><div id="loading"><img height="20px" src="/img/loading.gif"></div><button type="button" class="btn btn-default" data-dismiss="modal">Close</button><button id="status-user" class="btn btn-primary" onclick="smhMEM.updateStatus(' + pid + ',\'' + email + '\',' + status_update + ',' + uid + ')">' + status_text + '</button>';
        $('#smh-modal .modal-footer').html(footer);
    },
    //Do update status
    ...
    destroySession: function (pid, uid, name) {
        ...
        footer = '<div id="pass-result"></div><div id="loading"><img height="20px" src="/img/loading.gif"></div><button type="button" class="btn btn-default edit-user-close" data-dismiss="modal">Close</button><button id="delete-user" class="btn btn-primary" onclick="smhMEM.doDestroySession(' + pid + ',' + uid + ')">Destroy Session</button>';
        $('#smh-modal .modal-footer').html(footer);
    },
    //Do Destroy Session
    ...
    //Delete Modal
    deleteUser: function (pid, email, name) {
        smhMain.resetModal();
        var header, content, footer;
        $('.smh-dialog').css('width', '500px');
        $('#smh-modal').modal({
        ...
        footer = '<div id="pass-result"></div><div id="loading"><img height="20px" src="/img/loading.gif"></div><button type="button" class="btn btn-default edit-user-close" data-dismiss="modal">Close</button><button id="delete-user" class="btn btn-primary" onclick="smhMEM.removeUser(' + pid + ',\'' + email + '\')">Delete</button>';
        $('#smh-modal .modal-footer').html(footer);
    },
    //Do delete user
    ...
    exportMetaData: function () {
        if (total_entries) {
            window.location = '/apps/platform/metadata/export.metadata.php?pid=' + sessInfo.pid + '&ks=' + sessInfo.ks + '&page_size=' + total_entries + '&action=export_mem_user_metadata';
        }
    },
            ...
            success: function (data) {
                ...
                $('#concurrent-logins').change(function () {
                    if ($(this).prop('checked')) {
                        smhMEM.update_concurrent_status(1);
                    } else {
                        smhMEM.update_concurrent_status(0);
                    }
                });
            }
        });
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#activation-step').change(function () {
                    if ($(this).prop('checked')) {
                        smhMEM.update_activation_skip_status(1);
                    } else {
                        smhMEM.update_activation_skip_status(0);
                    }
                });
            }
        });
    ...
    registerActions: function () {
        ...
        $('#smh-modal').on('click', '.edit-user-close', function () {
            $('#edit-mem-user input[type="text"]').tooltipster('destroy');
        });
...
$(document).ready(function () {
    ...
    smhMEM.get_concurrent_status();
    smhMEM.get_activation_skip_status();
    smhMEM.get_owner_attrs();
});
