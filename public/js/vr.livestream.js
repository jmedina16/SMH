...
LiveStreams.prototype = {
    ...
    previewEmbed: function (entryId, name, platforms) {
        ...
        if ($.inArray("CONTENT_MANAGE_EMBED_CODE", sessPerm) != -1) {
            ...
            vr_fallback = '<div id="vr-options">' +
                    '<div style="margin-top: 10px; font-weight: bold;"><span style="color: #444; font-size: 12px; padding-top: 15px; margin-right: 15px;">Fallback EntryId:</span><span style="font-weight: normal; font-size: 15px;"><a href="#" id="update-fb-eid" onclick="smhLS.selectFBModal();">None Selected</a></span></div>' +
        ...
        if (smh) {
            if ($.inArray("CONTENT_MANAGE_EMBED_CODE", sessPerm) != -1) {
                gen = smhLS.generateEmbed(uiconf_id, entryId, width, height, protocol, delivery, format, embed, seo, err_comp, name, false, sizing, ratio);
                embedCode = gen.getCode();
                $('#embed_code').text(embedCode);
            }

            player_prev_gen = smhLS.generateEmbed(uiconf_id, entryId, width, height, 'https', delivery, format, embed, seo, err_comp, name, true, sizing, ratio);
    ...
    wizardPreviewEmbed: function (entryId, name, platforms) {
        ...
        var twitch = false;
        if (services.sn == '1') {
            if ((platforms !== '') && (yt_ready || fb_ready)) {
                var select_options = '';
                var options = platforms.split(';');
                var youtube_id = '';
                var twitch_id = '';
    ...
    selectFBModal: function () {
        ...
        var tree_flavors = smhLS.json_tree(flavors, 'flavors');
    ...
    returnTable: function () {
        ...
        $('#livestream-table-wrapper').html('<div id="users-buttons">' +
                ...
                '<span style="display: inline-block; float: left;"><button class="btn btn-block bg-olive" id="add-livestream" onclick="smhLS.addLiveStream();" ' + add_livestream_disabled + '>Create Live VR Stream</button></span>' +
                '<span class="dropdown header dropdown-accordion">' +
                '<div class="btn-group">' +
    ...
    registerActions: function () {
        ...
        $('#smh-modal2').on('change', '#smh', function (event) {
            if (!$(this).prop('checked')) {
                if (!$('#smh-modal2 #youtube').prop('checked') && !$('#smh-modal2 #facebook').prop('checked')) {
        ...
        $('#smh-modal2').on('change', '#youtube', function (event) {
            if (!$(this).prop('checked')) {
                if (!$('#smh-modal2 #smh').prop('checked') && !$('#smh-modal2 #facebook').prop('checked')) {
                    $('#smh-modal2 #smh').bootstrapToggle('on');
                }
        ...
        $('#smh-modal2').on('change', '#facebook', function (event) {
            if (!$(this).prop('checked')) {
                if (!$('#smh-modal2 #smh').prop('checked') && !$('#smh-modal2 #youtube').prop('checked')) {
                    $('#smh-modal2 #smh').bootstrapToggle('on');
                }
            } else if ($(this).prop('checked')) {
